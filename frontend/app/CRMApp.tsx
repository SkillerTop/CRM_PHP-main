...
import { ApiError, apiDownload, apiMessage, apiRequest, setCsrfToken } from "./api-client";
import { Avatar, EntityLogo } from "./components/identity";
import { useUrlStringState } from "./hooks/use-url-string-state";
...
function formatKyivDateTimeInput(value: string) {
  const parts = kyivDateTimeParts(value);
...
export function CRMApp() {
  ...
  async function loadWorkspace() {
    const response = await apiRequest<{ data: WorkspacePayload }>("/app/bootstrap");
    const payload = response.data;
    setCsrfToken(payload.csrf_token);
    const mappedContacts = payload.contacts.map(mapContact);
    const mappedCompanies = payload.companies.map(mapCompany).map((company) => ({
      ...company,
    ...
    setTaskComments(payload.comments.map((comment) => ({ id: apiString(comment, "id"), taskId: apiString(comment, "task_id"), author: apiString(comment, "author"), createdAt: formatKyivDateTime(apiString(comment, "created_at")), text: apiString(comment, "text"), authorUserId: apiNumber(comment, "author_user_id"), isHidden: apiBoolean(comment, "is_hidden") })));
    setTaskAttachments((payload.attachments ?? []).map((attachment) => ({ id: Number(attachment.id), taskId: apiString(attachment, "task_id"), name: apiString(attachment, "original_name"), mimeType: apiString(attachment, "mime_type"), sizeBytes: Number(attachment.size_bytes ?? 0), authorUserId: Number(attachment.author_user_id), author: apiString(attachment, "author_name"), createdAt: formatKyivDateTime(apiString(attachment, "created_at")) })));
    ...
    setArchivedTaskIds(payload.tasks.map(mapTask).filter((task) => task.isArchived).map((task) => task.id));
  ...
  return (
    <>
      <div className="page-actions-row">
        <p className="page-description">{canMove ? "Drag a company card to another column or use its status selector. On mobile, the board scrolls horizontally." : "Browse every stage of the relationship workflow. Your role has view-only access."}</p>
      ...
      <section className="kanban" aria-label="Relationship workflow board">
        {statusOrder.map((status) => {
          ...
          return (
            <div className="kanban-column" key={status} onDragOver={(event) => { if (canMove) event.preventDefault(); }} onDrop={(event) => { event.preventDefault(); if (canMove && draggedCompanyId) moveCompany(draggedCompanyId, status); setDraggedCompanyId(null); }}>
              ...
              <div className="kanban-stack">
                {items.map((company) => (
                  <article key={company.id} className="pipeline-card" tabIndex={0} draggable={canMove} onDragStart={(event) => { setDraggedCompanyId(company.id); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", company.id); }} onDragEnd={() => setDraggedCompanyId(null)} onClick={() => openCompany(company)} onKeyDown={(event) => { if (event.key === "Enter" && event.target === event.currentTarget) openCompany(company); }}>
...
function Audit({ events, users, canExport }: { events: AuditEvent[]; users: CRMUser[]; canExport: boolean }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    void apiRequest<{ data: ApiRecord[]; meta: ApiMeta }>(`/audit?${params}`).then((response) => {
      if (!active) return;
  ...
  return (
    <section className="panel data-panel">
      ...
      <div className="data-toolbar audit-toolbar"><div className="toolbar-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the current page" aria-label="Search the current audit page" /></div><label><span>From</span><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label><label><span>To</span><input type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label><select value={actor} onChange={(event) => setActor(event.target.value)} aria-label="User"><option>All users</option>{users.filter((user) => user.id).map((user) => <option key={user.id} value={String(user.id)}>{user.name}</option>)}</select><select value={action} onChange={(event) => setAction(event.target.value)} aria-label="Event type"><option>All actions</option>{actionOptions.map((value) => <option key={value}>{value}</option>)}</select><select value={entityType} onChange={(event) => setEntityType(event.target.value)} aria-label="Entity"><option>All entities</option>{entityOptions.map((value) => <option key={value}>{value}</option>)}</select><button className="text-button" type="button" onClick={() => { setQuery(""); setActor("All users"); setAction("All actions"); setEntityType("All entities"); setFrom(""); setTo(""); setPage(1); }}>Clear</button><span className="toolbar-spacer" />{canExport && <button className="secondary-button" type="button" onClick={exportCsv}>Export CSV</button>}</div>
...
function ContactForm({ companies, sourceOptions, initiatorOptions, initialCompanyId, currentUserEmail, onClose, onSubmit }: { companies: Company[]; sourceOptions: string[]; initiatorOptions: CRMUser[]; initialCompanyId?: string; currentUserEmail: string; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>, photoDataUrl: string) => Promise<boolean> }) {
  ...
  return (
    <Modal title="Add contact manually" eyebrow="Contacts" onClose={onClose}>
      <form onSubmit={(event) => { if (imageProcessing) event.preventDefault(); else { setSubmitting(true); setRetry(false); void onSubmit(event, photoDataUrl).then((saved) => setRetry(!saved)).finally(() => setSubmitting(false)); } }} className="entity-form">
        <ImageField label="Contact photo" name="contactPhoto" value={photoDataUrl} onChange={setPhotoDataUrl} onProcessingChange={setImageProcessing} />
        ...
        <div className="modal-actions field-full"><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit" disabled={imageProcessing || submitting}>{imageProcessing ? "Processing image…" : submitting ? "Saving…" : retry ? "Try again" : "Create contact"}</button></div>
      </form>
    </Modal>
  );
}
  ...
  return (
    <Modal title="New user" eyebrow="Team access" onClose={onClose}>
      <form className="entity-form" onSubmit={(event) => { setSubmitting(true); setRetry(false); void onSubmit(event).then((saved) => setRetry(!saved)).finally(() => setSubmitting(false)); }}>
        ...
        {delivery === "temporary_password" && <label className="field field-full"><span>Temporary password *</span><input name="temporaryPassword" type="password" required minLength={8} maxLength={128} defaultValue={TEST_PASSWORD} autoComplete="new-password" /></label>}
