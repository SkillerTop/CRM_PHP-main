...
final class AuditLogger
{
    ...
    public function logDiff(
        ...
        foreach ($fieldLabels as $field => $label) {
            ...
            $oldText = self::redactValue($field, $oldText);
            $newText = self::redactValue($field, $newText);
    ...
    public static function redactEvent(array $row): array
    {
        $field = (string) ($row['field_name'] ?? '');
        if (array_key_exists('value_from', $row)) {
            $row['value_from'] = self::redactValue($field, $row['value_from']);
        }
        if (array_key_exists('value_to', $row)) {
            $row['value_to'] = self::redactValue($field, $row['value_to']);
        }
        if (isset($row['detail_json']) && is_string($row['detail_json']) && $row['detail_json'] !== '') {
            $detail = json_decode($row['detail_json'], true);
            if (is_array($detail)) {
                foreach (['error', 'hint', 'exception', 'debug'] as $key) {
                    if (array_key_exists($key, $detail)) {
                        $detail[$key] = '[redacted]';
                    }
                }
                $row['detail_json'] = json_encode($detail, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            }
        }
        return $row;
    }

    private static function redactValue(string $field, mixed $value): mixed
    {
        $normalized = strtolower(str_replace([' ', '-'], '_', $field));
        $sensitive = str_contains($normalized, 'photo')
