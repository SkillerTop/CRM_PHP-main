...
final class AppController
{
    ...
    public function bootstrap(): never
    {
        ...
        $archiveWhere = $admin ? '' : ' WHERE c.is_archived = 0';
        ...
        $contactWhere = $admin ? '' : ' WHERE k.is_archived = 0 AND c.is_archived = 0';
        ...
        $taskWhere = $admin ? '' : ' WHERE t.is_archived = 0 AND c.is_archived = 0';
        $tasks = $this->db->query(
        ...
        $comments = $this->db->query(
            "SELECT tc.* FROM task_comments tc JOIN tasks t ON t.id = tc.task_id
             ...
             WHERE t.is_archived = 0 AND c.is_archived = 0{$commentWhere}
        ...
        $attachmentRows = $this->db->query(
            'SELECT ta.* FROM task_attachments ta
             ...
             WHERE ta.deleted_at IS NULL AND t.is_archived = 0 AND c.is_archived = 0
