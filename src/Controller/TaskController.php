...
final class TaskController
{
    ...
    {
        $this->find($taskId, true);
        $stmt = $this->db->prepare(
            'SELECT tc.*, u.full_name AS author_current_name FROM task_comments tc
             LEFT JOIN users u ON u.id = tc.author_user_id WHERE tc.task_id = :task_id ORDER BY tc.created_at, tc.id'
        );
    ...
    public function uploadAttachment(Request $request, int $taskId): never
    {
        ...
        if ($size <= 0 || $size > $maximum) {
            throw new ApiException(400, 'file_too_large', 'Размер файла должен быть не более 20 МБ.');
        }
    ...
    {
        $where = ['t.is_archived = 0', 'c.is_archived = 0'];
        $params = [];
    ...
    {
        $task = EntityMapper::task($this->find($id, true));
        ...
        $comments = $this->db->prepare('SELECT * FROM task_comments WHERE task_id = :task_id ORDER BY created_at, id');
